...
parser.add_argument("--f", help="specify the file *.gcode")
parser.add_argument("--w", help="max position on X axis")
parser.add_argument("--h", help="max position on Y axis")
...
init += "G1 F300\n"
init += "M18\n"
init += "M0 remove pen support\n"
init += "G28\n"
init += f"G1 Z0\n"
init += "M18\n"
init += "M0 set origin for X,Y,Z \n"
init += "M17 ;enable steppers\n"
init += "G4 P3000 ; wait 3 seconds\n"
init += f"G92 X0 Y0 Z{additionalPush} ;represent the additional pressure\n"
init += f"G0 X0 Y0 Z5\n"
# init += f"G0 X100 Y100 F{speedXY}\n"
# init += f"G0 Z0 F{speedZ}\n"
init += f"G0 F{speedXY}\n"
...
def main():
    ...
    f = open(os.getcwd() + "/" + mFile, "r")
    fout = open(os.getcwd() + "/" + mFile + "-cutted.gcode", "w+")
    lines = f.readlines()
    ...
    print(
        f"Default limits will be x[{minX}, {maxX}], y[{minY}, {maxY}], z[{maxZ}] \n")
    ...
    for line in lines:
        ...
        if(currentOutside == True and prevOutside == False):
            ...
            if (intersectPt[0] < minX or intersectPt[0] > maxX or intersectPt[1] < minY or intersectPt[1] > maxY):
                ...
                intersectPt = getIntersectionPoint(
                    intersectPt[0], intersectPt[1], lineEq)
                print(intersectPt)

            print(
                f"[->] Going outside at line {line_num}, from {prevX, prevY}, to { currentX, currentY },  intersect is : ", intersectPt)
            fout.write("G0 X" + str(round(intersectPt[0], 2)) + " Y" + str(
                round(intersectPt[1], 2)) + "; added Point\n")
        ...
        if(currentOutside == False and prevOutside == True):
            ...
            print(
                f"[<-] Coming from outside at line {line_num}, from {prevX, prevY}, to {currentX, currentY}, intersect is : ", intersectPt)
            fout.write("G0 X" + str(round(intersectPt[0], 2)) + " Y" + str(
                round(intersectPt[1], 2)) + "; added Point\n")
    ...
    fout = open(os.getcwd() + "/" + mFile + "-cutted.gcode", "rb")
