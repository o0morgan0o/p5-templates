...
var multi_exporter = {
  ...
  prefix: "",
  suffix: "",
  base_path: "E:/code/my-sketches/printedSvg/separations",
  transparency: false,
  embedImage: false,
  embedFont: false,
  trimEdges: false,
  ...
  formatInfo: [
    ...
    },
    {
      ...
      saveFile: savePdf,
      activeControls: ["trimEdgesCheckBox"],
    },
    {
      name: "JPG",
      copyBehaviour: false,
      pixelDocSize: true,
      getOptions: getJpgOptions,
      saveFile: saveJpg,
      activeControls: [
        "scalingInput",
        "trimEdgesCheckBox",
    ...
    },
    {
      ...
      copyBehaviour: true,
      pixelDocSize: false,
      getOptions: getSvgOptions,
      saveFile: saveSvg,
      activeControls: ["embedImageCheckBox", "trimEdgesCheckBox"],
    },
    {
      name: "FXG 1.0",
      copyBehaviour: true,
      pixelDocSize: false,
      getOptions: getFxg1Options,
      saveFile: saveFxg,
      activeControls: ["trimEdgesCheckBox"],
    },
    {
  ...
  artboardSelect: [
    { code: "all", name: "All Artboards (except those beginning with - )" },
    { code: "current", name: "Current Artboard" },
    { name: "---" },
  ],

  layerSelect: [
    { code: "all", name: "All Layers (except those beginning with - )" },
  ...
  load_prefs: function () {
    ...
    try {
      ...
      saved_data.appendChild(
        new XML("<nyt_base_path>~/Desktop</nyt_base_path>")
      );
      saved_data.appendChild(new XML("<nyt_scaling>100%</nyt_scaling>"));
      saved_data.appendChild(
        new XML("<nyt_transparency>true</nyt_transparency>")
      );
      ...
      saved_data.appendChild(
        new XML("<nyt_innerPadding>false</nyt_innerPadding>")
      );
      ...
      saved_data.appendChild(
        new XML("<nyt_exportArtboards>false</nyt_exportArtboards>")
      );
      saved_data.appendChild(
        new XML("<nyt_ignoreWarnings>false</nyt_ignoreWarnings>")
      );
    ...
    if (this.multiExporterPrefs.textFrames.length != 1) {
      Window.alert(
        "Please delete the " + this.PREFS_LAYER_NAME + " layer and try again."
      );
    } else {
      try {
        this.prefs_xml = new XML(
          this.multiExporterPrefs.textFrames[0].contents
        );
        ...
        this.transparency =
          this.prefs_xml.nyt_transparency == "true" ? true : false;
        this.embedImage =
          this.prefs_xml.nyt_embedImage == "true" ? true : false;
        this.ignoreWarnings =
          this.prefs_xml.nyt_ignoreWarnings == "true" ? true : false;
        this.embedFont = this.prefs_xml.nyt_embedFont == "true" ? true : false;
        this.trimEdges = this.prefs_xml.nyt_trimEdges == "true" ? true : false;
        this.innerPadding =
          this.prefs_xml.nyt_innerPadding == "true" ? true : false;
        ...
        this.whole_artboard_mode =
          this.prefs_xml.nyt_exportArtboards == "true" ? true : false;
        ...
        Window.alert(
          "Please delete the this.multiExporterPrefs layer and try again."
        );
      }
    }
  ...
  show_dialog: function () {
    ...
    this.artboardList.selection = this.findDataIndex(
      this.artboards,
      this.artboardSelect
    );

    this.exportArtboardsCheckBox = row.add(
      "checkbox",
      undefined,
      "Export Artboard Images"
    );
    ...
    this.layerList.selection = this.findDataIndex(
      this.layers,
      this.layerSelect
    );
    ...
    var scalingTip = row.add(
      "statictext",
      undefined,
      "(Normally 100%; Use 200% for Retina display exports)"
    );
    ...
    this.dirEt = row.add(
      "edittext",
      undefined,
      "E:\\code\\my-sketches\\printedSvg\\separations"
    );
    ...
    this.innerPaddingCheckBox = row.add(
      "checkbox",
      undefined,
      "Inner Padding (to prevent curved edge clipping)"
    );
    ...
    var saveBtn = row.add("button", undefined, "Save and Close", {
      name: "saveClose",
    });
    ...
    this.artboardList.onChange = function () {
      multi_exporter.artboards = multi_exporter.getListData(
        multi_exporter.artboardList.selection.index,
        multi_exporter.artboardSelect
      );
      multi_exporter.update_export_desc();
    };
    this.layerList.onChange = function () {
      multi_exporter.layers = multi_exporter.getListData(
        multi_exporter.layerList.selection.index,
        multi_exporter.layerSelect
      );
      multi_exporter.update_export_desc();
    };
    this.exportArtboardsCheckBox.onClick = function () {
      multi_exporter.whole_artboard_mode =
        multi_exporter.exportArtboardsCheckBox.value;
      multi_exporter.update_export_desc();
    };
  ...
  checkFormat: function (progLabel) {
    ...
    for (var i = 0; i < this.controlNames.length; i++) {
      ...
      this[controlName].enabled =
        this.indexOf(formatInfo.activeControls, controlName) != -1;
    }
  },
  ...
  update_export_desc: function () {
    ...
    if (this.layers == "all") {
      ...
      for (var i = 0; i < docRef.layers.length; ++i) {
        var layer = docRef.layers[i];
        if (
          !this.isAdditionalLayer(layer) &&
          this.isIncludeLayer(layer) &&
  ...
  updateExportCounts: function () {
    this.num_to_export =
      this.export_artboards.length * this.export_layers.length;
    ...
    if (this.whole_artboard_mode) {
      ...
      if (this.export_layers.length) {
        artboardExportTxt =
          " (and " + this.export_artboards.length + " artboard images)";
      } else {
        this.progLabel.text =
          "Will export " +
          this.export_artboards.length +
          " of " +
    ...
    if (this.export_artboards.length > 1 && this.export_layers.length > 1) {
      this.progLabel.text =
        ...
        this.num_to_export +
        " files (" +
        this.export_layers.length +
        " layers * " +
        this.export_artboards.length +
        " artboards)" +
        artboardExportTxt;
    } else if (
      this.export_layers.length > 0 &&
      this.export_artboards.length == 1
    ) {
      ...
      this.export_layers.length > 0 &&
      this.export_artboards.length > 0
    ) {
      var artboard = docRef.artboards[this.export_artboards[0]];
      this.progLabel.text =
        "Will export " +
  ...
  run_export: function () {
    ...
    if (
      !this.export_artboards.length ||
      (!this.export_layers.length && !this.whole_artboard_mode)
    ) {
    ...
    for (var i = 0; i < this.export_artboards.length; i++) {
      ...
      if (this.whole_artboard_mode) {
        try {
          var base_filename =
            this.base_path + "/" + this.prefix + artboardName + this.suffix;
          if (copyBehaviour) {
            var offset = { x: offsetX, y: offsetY };
            copyDoc = this.copyDocument(
              docRef,
              formatInfo.pixelDocSize,
              artboard,
              rect,
              artW,
              artH,
              ...
              function (layer) {
                return (
                  layer.name != multi_exporter.PREFS_LAYER_NAME && layer.visible
                );
              }
            );
          ...
          try {
            ...
            if (layerRect[0] < layerRect[2] && layerRect[1] > layerRect[3]) {
              ...
              if (this.export_artboards.length == 1) {
                base_filename =
                  this.base_path + "/" + this.prefix + lyr_name + this.suffix;
              } else {
                base_filename =
                  this.base_path +
                  "/" +
              ...
              if (copyBehaviour) {
                if (this.trimEdges) {
                  ...
                  offset = {
                    x: offsetX + layerOffsetX,
                    y: offsetY + layerOffsetY,
                  };
                  var layerDepths = [];
                  var copyDoc = this.copyDocument(
                    docRef,
                    formatInfo.pixelDocSize,
                    artboard,
                    rect,
    ...
    if (
      (!this.failed_layers.length && !this.failed_artboards.length) ||
      !this.redoFailed(this.failed_layers, this.failed_artboards)
    ) {
      this.dlg.close();
    }
  },
  ...
  redoFailed: function (failed_layers, failed_artboards) {
    ...
    if (newLayers.length) {
      ...
      var msg =
        newArtboards.length + " artboards failed:" + artboardNames + "\nRetry?";
    }
    if (confirm(msg)) {
  ...
  isIncludeLayer: function (layer) {
    return (
      !layer.name.match(/^\+/) &&
      layer.name != this.PREFS_LAYER_NAME &&
      !layer.name.match(/^\-/)
    );
  },
  ...
  updateProgress: function (num_exported) {
    this.progLabel.text =
      "Exported " + num_exported + " of " + this.num_to_export;
  ...
  copy_layer: function (
    ...
    if (!offset.norm) {
      ...
      if (this.rect_equal(oldBounds, newBounds)) {
        ...
        if (!this.ignoreWarnings)
          alert(
            "Illustrator visibleBounds issue workaround.\nTry removing groups on layer '" +
              from_layer.name +
    ...
    if (to_layer.parent.artboards != null) {
      // only if top level layer
      try {
